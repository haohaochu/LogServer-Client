...
int CBlockingSocket::Send(const char* pch, const int nSize)
{
	ASSERT(m_hSocket != NULL);
	// returned value will be less than nSize if client cancels the reading
	FD_SET fd = {1, m_hSocket};

	switch ( select(0, NULL, &fd, NULL, NULL) )
	{
	case SOCKET_ERROR :
		{
			char	buf[128];
			sprintf(buf,"Send Error(%d)",WSAGetLastError());
			throw new CBlockingSocketException(buf);
		}break;
	case 0 :	// Send timeout
		return 0;
	}
...
int CBlockingSocket::Receive(char* pch, const int nSize, const int nSecs, const int nmilliSecs)
{
	ASSERT(m_hSocket != NULL);
	FD_SET	fd = {1, m_hSocket};
	TIMEVAL	tv = {nSecs, nmilliSecs};
	char	buf[128];
	...
	case SOCKET_ERROR :
		{		
			sprintf(buf,"Receive Error(%d)",WSAGetLastError());
			throw new CBlockingSocketException(buf);
		}break;
	case 0 :	// Receive timeout
		return 0;
	...
	{
		sprintf(buf,"Receive Error(recv:%d)",WSAGetLastError());
		throw new CBlockingSocketException(buf);
	}
	else if ( nBytesReceived == 0 )
		throw new CBlockingSocketException("Receive - Close Socket by remote!");
...
int CBlockingSocket::Receive(char* pch, const int nSize, const int nSecs)
{
	...
	case SOCKET_ERROR :
		{			
			sprintf(buf,"Receive Error(select:%d)",WSAGetLastError());
			throw new CBlockingSocketException(buf);
		}break;
	case 0 :	// Receive timeout
		return 0;
...
int CBlockingSocket::ReceiveDatagram(char* pch, const int nSize, LPSOCKADDR psa, const int nSecs)
{
	...
	case SOCKET_ERROR :
		{
			char	buf[128];
			sprintf(buf,"ReceiveDatagram Error(%d)",WSAGetLastError());
			throw new CBlockingSocketException(buf);
		}break;
	case 0 :	// Receive timeout
		return 0;
...
//		throw new CBlockingSocketException("ReceiveDatagram");	// Mark By Jack Lee20030623
	}
	return nBytesReceived;
}

int CBlockingSocket::ReceiveDatagram(char* pch, const int nSize, LPSOCKADDR psa, const int nSecs, const int nmilliSecs)
{
	ASSERT(m_hSocket != NULL);
	FD_SET fd = {1, m_hSocket};
	TIMEVAL tv = {nSecs, nmilliSecs};
...
//	}
	...
	case SOCKET_ERROR :
		{
			char	buf[128];
			sprintf(buf,"SendDatagram Error(%d)",WSAGetLastError());
			throw new CBlockingSocketException(buf);
		}break;
	case 0 :	// Receive timeout
		return 0;
...
BOOL CBlockingSocket::SetSockOpt(int nOptionName, const void *lpOptionValue, int nOptionLen, int nLevel)
{
	ASSERT(m_hSocket);
...
BOOL CBlockingSocket::GetSockOpt(int nOptionName, void *lpOptionValue, int *lpOptionLen, int nLevel)
{
	ASSERT(m_hSocket);

	if ( getsockopt(m_hSocket,nLevel,nOptionName,(char FAR*)lpOptionValue,lpOptionLen) != SOCKET_ERROR )
		return TRUE;
	else
	{
